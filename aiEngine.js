...
(function () {
  ...
  function normalizeData(raw) {
    ...
    return list.map((draw, index) => {
      ...
      return {
        draw: draw.draw || draw.round || index + 1,
        date: draw.date || draw.drawDate || "",
        numbers,
        bonus: Number(draw.bonus) || null
      };
  ...
  function buildStats(data) {
    ...
    for (let n = 1; n <= 43; n++) {
      ...
      ranking.push({ number: n, count: count[n], recent: recentCount[n], gap });
    }
  ...
  function scoreNumbers(data, mode) {
    ...
    for (let n = 1; n <= 43; n++) {
      ...
      let score;
      if (mode === "hot") score = frequencyScore * 1.2 + recentScore + coldScore * 0.3;
      else if (mode === "cold") score = frequencyScore * 0.6 + recentScore * 0.3 + coldScore * 1.4;
      else score = frequencyScore + recentScore * 0.8 + coldScore * 0.8;
  ...
  function isBalanced(numbers, latest) {
    ...
    return { sum, odd, even: 6 - odd, low, high: 6 - low, overlap,
      ok: sum >= 85 && sum <= 175 && odd >= 2 && odd <= 4 && low >= 2 && low <= 4 && overlap <= 2 };
  }
  ...
  function makePredictions(data) {
    ...
    return modes.map(item => {
      ...
      let confidence = Math.round(Math.min(92, 45 + avgScore + history.length * 0.4));
      if (balance.ok) confidence += 3;
      confidence = Math.min(95, confidence);
      return { name: item.name, label: item.label, numbers, confidence, balance };
    });
  }
